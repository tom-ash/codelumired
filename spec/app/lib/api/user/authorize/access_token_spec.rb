...
RSpec.describe ::Api::User::Authorize::AccessToken do
  ...
  describe 'PUT' do
    let!(:user) { create(:mapawynajmu_pl_user, account_type: account_type, first_name: first_name, encrypted_access_token: encrypted_access_token) }

    before { put '/mapawynajmu_pl/user/authorize/access-token', headers: { 'Access-Token': access_token } }
